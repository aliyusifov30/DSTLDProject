...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20220306215804_ProductGenderTableCreated')
BEGIN
    CREATE TABLE [ProductGenders] (
        ...
        [ProductId] int NOT NULL,
        [GenderId] int NOT NULL,
        CONSTRAINT [PK_ProductGenders] PRIMARY KEY ([Id]),
        CONSTRAINT [FK_ProductGenders_Genders_GenderId] FOREIGN KEY ([GenderId]) REFERENCES [Genders] ([Id]) ON DELETE CASCADE,
        CONSTRAINT [FK_ProductGenders_Products_ProductId] FOREIGN KEY ([ProductId]) REFERENCES [Products] ([Id]) ON DELETE CASCADE
    );
END;
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20220306215804_ProductGenderTableCreated')
BEGIN
    CREATE INDEX [IX_ProductGenders_GenderId] ON [ProductGenders] ([GenderId]);
END;
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20220306215804_ProductGenderTableCreated', N'3.1.22');
END;
